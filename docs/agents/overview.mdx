...
# Invite an agent with no instance-scoped capabilities — useful when the
# agent should only operate within a specific vault at proxy role.
agent-vault agent invite my-agent --role no-access --vault payments:proxy
```
