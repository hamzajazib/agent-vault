...
<Tabs>
  <Tab title="Shell">
    ...
    `agent-vault run` automatically configures OpenClaw's managed proxy (`proxy.enabled`) and trusted env proxy for `web_fetch`, sets `OPENCLAW_PROXY_URL`, `HTTPS_PROXY`, `HTTP_PROXY`, and CA trust variables, and installs Agent Vault skills. No manual OpenClaw configuration needed.
