...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--host` | | Target service host. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), or an inline path-scoped form (`slack.com/api/*`). The server splits the path off the host on ingest. |
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault proposal create">
    ...
    | `--host` | | Target service host. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), or an inline path-scoped form (`slack.com/api/*`). |
